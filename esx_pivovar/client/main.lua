...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
			...
			align    = 'top-left',
			elements = {
				{label = _U('pivovar_clothes_civil'), value = 'citizen_wear'},
				{label = _U('pivovar_clothes_pivovar'), value = 'pivovar_wear'},
			},
		}, function(data, menu)
			...
			if data.current.value == 'pivovar_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'pivovar_actions_menu'
...
function OpenVigneActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pivovar_actions', {
			title    = 'Pivovar',
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'pivovar', vehicleProps)
				end, function(data, menu)
		...
		local elements = {
			{label = 'Garáž s auty',  value = 'speedo'},
		}
...
function OpenMobileVigneActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_pivovar_actions', {
			title    = 'pivovar',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_pivovar', 'Pivovar', amount)
								end)
							end
						end
					end, function(data, menu)
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_pivovar:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_pivovar:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_pivovar:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_pivovar:putStockItems', itemName, count)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
...
AddEventHandler('esx_pivovar:hasEnteredMarker', function(zone)
	if zone == 'RaisinFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementVin' and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement_nealko')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'PivovarActions' and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
		CurrentAction     = 'pivovar_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
...
AddEventHandler('esx_pivovar:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'RaisinFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
		TriggerServerEvent('esx_pivovar:stopHarvest')
	end  
	if (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
		TriggerServerEvent('esx_pivovar:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
		TriggerServerEvent('esx_pivovar:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.PivovarActions.Pos.x, Config.Zones.PivovarActions.Pos.y, Config.Zones.PivovarActions.Pos.z)
		...
		AddTextComponentString("Pivovar")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_pivovar:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_pivovar:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  38) and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_pivovar:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_pivovar:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_pivovar:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_pivovar:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'pivovar_actions_menu' then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'pivovar', vehicleProps)
					end
		...
		if IsControlPressed(0,  167) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'pivovar' and (GetGameTimer() - GUI.Time) > 150 then
